<script lang="ts">
  ...
  import {
    HandleStatusEnum,
    type HandleStatus,
  } from "$lib/components/Node/handle-status.enum";
  ...
  const outputNodeBluePrints: Writable<DnDContext[]> = writable([
    {
      ...
      nodeBlueprint: { id: "output", name: "Output Node", parameters: [] },
    },
  ] as DnDContext[]);
  ...
  onMount(async () => {
    availableLayers.set(
      (await BackendApi.getAvailableLayers()).map(
        (layer: LayerBlueprint<any>) =>
          ({ type: NodeTypeEnum.Layer, nodeBlueprint: layer }) as DnDContext
      )
    );
  ...
  function constructNodeChain(): Node[] {
    ...
    $nodes.forEach((node) => {
      if (node.data?.leftStatus) {
        (node.data.leftStatus as Writable<HandleStatus>).set(
          HandleStatusEnum.default
        );
      }
      if (node.data?.rightStatus) {
        (node.data.rightStatus as Writable<HandleStatus>).set(
          HandleStatusEnum.default
        );
      }
    });
    ...
    const nodeIdChain: string[] = constructIdChain(inputNode.id, [
      inputNode.id,
    ]);
    const nodeChain: Node[] = nodeIdChain.map(
      (nodeId) => $nodes.find((node) => node.id === nodeId) as Node
    );
    return nodeChain;
  }
  ...
  async function validateNodes(nodeChain: Node[]): Promise<void> {
    ...
    for (let i = 1; i < nodeChain.length; i++) {
      ...
      let previousNodeOutputSize: TensorSize = get(
        previousNode.data?.outputSize as Writable<TensorSize>
      );
      let currentNodeOutputSize = await BackendApi.getLayerOutputSize(
        ...
        deconstructParameters(
          get(
            currentNode.data?.parameters as Writable<
      ...
      (currentNode?.data.outputSize as Writable<TensorSize>).set(
        currentNodeOutputSize
      );

      let currentNodeMaxDimension: number = get(
        currentNode.data?.inputs as Writable<LayerInput[]>
      )[0]?.max_dimensions as number;
      let currentNodeMinDimension: number = get(
        currentNode.data?.inputs as Writable<LayerInput[]>
      )[0]?.min_dimensions as number;
      if (
        (previousNodeOutputSize?.length >= currentNodeMinDimension ||
          currentNodeMinDimension === null) &&
        (previousNodeOutputSize?.length <= currentNodeMaxDimension ||
          currentNodeMaxDimension === null)
      ) {
        (previousNode.data?.rightStatus as Writable<HandleStatus>).set(
          HandleStatusEnum.success
        );
        (currentNode.data?.leftStatus as Writable<HandleStatus>).set(
          HandleStatusEnum.success
        );
      } else {
        (previousNode.data?.rightStatus as Writable<HandleStatus>).set(
    ...
    <div class="edit-architectures-page__header-left">
      {#if $isEditingTitle}
        ...
        <Button
          ...
          on:click={() => isEditingTitle.set(false)}
      ...
      {:else}
        ...
        <Button
          style={StylingUtility.defaultButton}
          on:click={() => isEditingTitle.set(true)}

<script lang="ts">
  ...
  $effect(() => {
    const interval = setInterval(async () => {
      ...
      availableTasks = await Promise.all(
        availableTrainingTasks.map(
          async (info) => await BackendApi.getTrainingTaskByID(info.id)
        )
      );
    }, 1000);
  ...
  let onSelectTask = (value: number) => {
    selectedTask = availableTasks.filter((task) => task.id == value)[0]
      ?.content;
  };
</script>
      ...
      <Button
        ...
        on:click={() => {
          availableTasks.forEach((tsk) => {
            if (
              tsk.content.data.status === "complete" ||
              tsk.content.data.status === "failed"
            ) {

...
function getTransformParametersByTransformId(
  transformId: TransformId
): Promise<Parameter<any>[]> {
...
async function getSourceParametersBySourceId(
  sourceId: SourceId
): Promise<Parameter<any>[]> {
  ...
  await Promise.all(
    elements.map(async (element: PipelineElement) => {
      ...
      if (element.type === "source") {
        ...
        elementParameters = await getTransformParametersByTransformId(
      ...
      nodes.push({
        ...
        data: {
          color: writable<string>(
            layout.nodes[element.instance_id]?.metadata.color ?? "#FFFFFF"
          ),
          title: writable<string>(
            layout.nodes[element.instance_id]?.metadata.title ??
              "Untitled Layer"
          ),
          name: writable<string>(
            layout.nodes[element.instance_id]?.metadata.name ?? ""
          ),
          leftConnected: writable<boolean>(
            layout.nodes[element.instance_id]?.metadata.leftConnected ?? false
          ),
          rightConnected: writable<boolean>(
            layout.nodes[element.instance_id]?.metadata.rightConnected ?? false
          ),
          maxInputSize: writable<number>(
            layout.nodes[element.instance_id]?.metatdata?.maxInputSize ?? 0
          ),
          minInputSize: writable<number>(
            layout.nodes[element.instance_id]?.metadata?.minInputSize ?? 0
          ),
          ...(element.type === "source"
            ? { src_id: element.src_id }
            : { transform_id: element.transform_id }),
          parameters: writable<
            { parameter: Parameter<any>; value: ParameterValue<any> }[]
          >(
        ...
        position: {
          x: layout.nodes[element.instance_id]?.x ?? 0,
          y: layout.nodes[element.instance_id]?.y ?? 0,
        },
      ...
      if (labelOutput > 0) {
        nodes.push({
          ...
          data: {
            leftConnected: writable<boolean>(
              layout.nodes["71"]?.metadata.leftConnected ?? false
            ),
            leftStatus: writable<string>(
              layout.nodes["71"]?.metadata.leftStatus ??
                HandleStatusEnum.default
            ),
          },
          position: {
            x: layout.nodes["71"]?.x ?? 0,
            y: layout.nodes["71"]?.y ?? 0,
          },
          dragHandle: ".node__header",
        } satisfies Node);
      }
      if (valueOutput > 0) {
        nodes.push({
          ...
          data: {
            leftConnected: writable<boolean>(
              layout.nodes["70"]?.metadata.leftConnected ?? false
            ),
            leftStatus: writable<string>(
              layout.nodes["70"]?.metadata.leftStatus ??
                HandleStatusEnum.default
            ),
          },
          position: {
            x: layout.nodes["70"]?.x ?? 0,
            y: layout.nodes["70"]?.y ?? 0,
          },
          dragHandle: ".node__header",
        } satisfies Node);
      }
...
const createPipelineStore = (): PipelineStore => {
  ...
  const loadPipelineById = async (id: PipelineId): Promise<void> => {
    try {
      ...
      const { nodes, edges } = await parseLayersIntoNodesAndEdges(
        pipData.elements,
        layoutData,
        pipData.label_output,
        pipData.value_output
      );
      update((store) => ({
        ...store,
        activePipeline: {
  ...
  const deletePipeline = async (id: PipelineId): Promise<boolean> => {
    let status = await fetch(`${BACKEND_API_BASE_URL}/pipeline/delete`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const saveActivePipeline = async (
    isNew: boolean = false
  ): Promise<{ id?: PipelineId; success: boolean }> => {
    const pStore = get(pipelineStore);
    if (!pStore.activePipeline) {
      return { success: false }; // Return false if no active pipeline exists
    }
    ...
    const pipeline: NetworkPipelineDescription = {
      ...
      content: {
        data: {
          ...
          value_output: valueOutputNode
            ? (parseInt(
                edges.filter((edge) => edge.target === valueOutputNode.id)[0]
                  ?.source ?? 0
              ) as InstanceId)
            : -1,
          label_output: labelOutputNode
            ? (parseInt(
                edges.filter((edge) => edge.target === labelOutputNode.id)[0]
                  ?.source ?? 0
              ) as InstanceId)
            : -1,
        } as PipelineConfig,
    ...
    try {
      const response = await fetch(
        `${BACKEND_API_BASE_URL}/pipeline/${isNew ? "create" : "update"}`,
        {
          method: "POST",
          ...
          body: JSON.stringify(
            isNew
              ? {
                  meta: pipeline.content.meta,
                  data: pipeline.content.data,
                  layout: pipeline.content.layout,
                }
      ...
      if (isNew) {
        let id: PipelineId = (await response.json()).id;
  ...
  const createNewPipeline = async (
    ...
    return await saveActivePipeline(true);
  };

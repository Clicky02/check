<script lang="ts">
  ...
  const outputSize: Writable<TensorSize> =
    data?.outputSize as Writable<TensorSize>;
...
<Handle
  ...
  style="
            ...
            border-color: {'#FFFFFF' + (selected ? 'ff' : '69')};
            border-radius: 0;
            height: 8px;
            width: 10px;
        "
...
>
  ...
  <div class="node__header" on:click={handleToggleExpanded}>
    <div
      class="node__header-chevron"
      style="transform: rotate({$rotationDegrees}deg); transition: transform 0.2s;"
    >
  ...
  {#if $expanded}
    <div
      class="node__content"
      style="visibility: {$expanded
        ? 'visible'
        : 'hidden'}; transition: visibility 0.2s;"
    >
